...
def find_projective_correspondence(source_points,
    ...
    mask1 = np.where(target_us >= 0, 1, 0)
    mask2 = np.where(target_vs >= 0, 1, 0)
    mask3 = np.where(target_us < w, 1, 0)
    mask4 = np.where(target_vs < h, 1, 0)
    mask5 = np.where(target_ds > 0, 1, 0)
    ...
    d_source = T_source_points[:,2]
    d_target = target_vertex_map[target_vs, target_us][:,2]
    diff_source_target = np.abs(d_source - d_target)
    mask = np.where(diff_source_target < dist_diff, 1, 0).astype(bool)
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    abg_coeff = np.cross(p_prime, n_q)
    A = np.hstack((abg_coeff, n_q))
...
def solve(A, b):
    ...
    b = b.reshape(-1,1)
    print(b.shape)
    delta = np.linalg.lstsq(A, -b)[0].squeeze()
    print(delta)
    return delta
